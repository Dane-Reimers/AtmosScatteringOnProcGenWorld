...
//PURPOSE:	For square matrices. This is column major for OpenGL
inline void MultiplyMatrices4by4OpenGL_FLOAT_asm386(sreal *result, const sreal *matrix1, const sreal *matrix2)
{
	...
	for(i=0; i<n; i++)
	{
		for(j=0; j<n; j++)
		{
	result[i*n+j]=matrix1[i*n]*matrix2[j]+
				matrix1[i*n+1]*matrix2[j+4]+
				matrix1[i*n+2]*matrix2[j+8]+
				matrix1[i*n+3]*matrix2[j+12];
		}
	}*/
	__asm
	{
		mov		eax, dword ptr[result]
		mov		ebx, dword ptr[matrix1]
		mov		edx, dword ptr[matrix2]
		//mov		ecx, 0xffffffffh			;ecx is i=-1
		//mov		ebx, 0						;ebx is j
		////////////
		//finit
		//load first two floats into FPU stack for performance
		fld		dword ptr[ebx+16]				;don't forget, this is OpenGL matrix style!!! (column major)
		fld		dword ptr[ebx]
		//load first four floats into FPU stack for performance
		fld		dword ptr[edx+12]
		fld		dword ptr[edx+8]
		fld		dword ptr[edx+4]
		fld		dword ptr[edx]
		////////////
		fld		st(4)				;dword ptr[ebx]
		fmul	st(0), st(1)
		fld		st(6)				;dword ptr[ebx+16]
		fmul	st(0), st(3)
		faddp	st(1), st(0)
		...
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[0]
		fstp	dword ptr[eax]
		//////////////////////
		fld		dword ptr[ebx+4]
		fmul	st(0), st(1)
		...
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[1]
		...
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[2]
		...
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[3]
		...
		fld		dword ptr[ebx+48]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[4]
		...
		fld		dword ptr[ebx+52]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[5]
		...
		fld		dword ptr[ebx+56]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[6]
		...
		fld		dword ptr[ebx+60]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[7]
		...
		fld		dword ptr[ebx+48]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[8]
		...
		fld		dword ptr[ebx+52]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[9]
		...
		fld		dword ptr[ebx+56]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[10]
		...
		fld		dword ptr[ebx+60]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[11]
		...
		fld		dword ptr[ebx+48]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[12]
		...
		fld		dword ptr[ebx+52]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[13]
		...
		fld		dword ptr[ebx+56]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[14]
		...
		fld		dword ptr[ebx+60]
		fmul	st(0), st(5)
		faddp	st(1), st(0)		;answer to result[15]
		fstp	dword ptr[eax+60]
		finit						;clean the stack now
	}
}
...
//PURPOSE: matrix is column major, it is 16 elements
//         vertex is 4 elements (x, y, z, w)
//         resultVertex = vertex * matrix  and it is 4 elements (x, y, z, w)
inline void MultiplyVectorMatrix4by4_FLOAT_asm86_SSE_m1(sreal *resultVertex, const sreal *matrix, const sreal *vertex)
{
	__asm
	{
		mov		eax, vertex
		mov		ebx, matrix
		mov		edi, resultVertex
		movaps	xmm0, [eax]				; we will be needing vertex 4 times (cant do movaps xmm0, [vertex])
		movaps	xmm1, [ebx]
		mulps	xmm1, xmm0				; this is like a vector dot product
		movaps	xmm2, xmm1
		shufps	xmm2, xmm2, 0x4E
		addps	xmm1, xmm2
		movaps	xmm2, xmm1
		shufps	xmm2, xmm2, 0x11
		addps	xmm1, xmm2				; result in each elemenst of xmm1
		movss	[edi], xmm1
		/////////////////
		movaps	xmm1, [ebx+16]
		mulps	xmm1, xmm0				; this is like a vector dot product
...
//PURPOSE: matrix is column major, it is 16 elements
//         vertex is 4 elements (x, y, z, w)
//         resultVertex = matrix * vertex and it is 4 elements (x, y, z, w)
inline void MultiplyMatrixVertex4by4_FLOAT_asm86_SSE_m1(sreal *resultVertex, const sreal *matrix, const sreal *vertex)
{
	__asm
	{
		mov		eax, matrix
		mov		edx, resultVertex
		mov		ecx, vertex
		movaps	xmm0, [eax]						; aligned move, get first column
		movaps	xmm5, [ecx]						; aligned move, get entire vector
		movaps	xmm4, xmm5
